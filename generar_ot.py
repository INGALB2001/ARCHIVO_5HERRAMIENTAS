...
styles = {
    ...
    "titulo": ps("titulo", font="Helvetica-Bold", size=14, leading=18, color=BLANCO, align=TA_CENTER),
    "sub": ps("sub", font="Helvetica-Bold", size=10, leading=13, color=BLANCO, align=TA_CENTER),
    "w": ps("w", font="Helvetica-Bold", size=8, leading=10, color=BLANCO, align=TA_CENTER),
...
def generar_ot_pdf(data, out_path):
    ...
    header = Table([[header_text, logo]], colWidths=[TW - 30 * mm, 30 * mm])
    ...
    # DATOS GENERALES
    datos = Table([
        [p("FOLIO", "w"), p("FECHA / HORA", "w"), p("ESTADO", "w")],
        [p(folio, "cb"), p(fecha_hora, "c"), p(estado.upper(), "cb")],

        [p("SOLICITANTE", "w"), p("ASIGNADO A", "w"), p("ÁREA", "w")],
        [p(solicitante, "c"), p(asignado, "c"), p(area, "c")],

        [p("CÓDIGO / EQUIPO", "w"), p("", "w"), p("", "w")],
        [p(codigo_equipo, "c"), "", ""],
    ...
    bloques = Table([
        [p("DESCRIPCIÓN DEL SERVICIO", "w")],
        [p(descripcion_servicio, "n")],
        [p("TRABAJOS EFECTUADOS", "w")],
    ...
    bloques.setStyle(TableStyle([
        ...
        ("BACKGROUND", (0, 2), (0, 2), AZUL),
        ("BACKGROUND", (0, 4), (0, 4), AZUL),
        ("GRID", (0, 0), (-1, -1), 0.35, colors.grey),
        ("BOX", (0, 0), (-1, -1), 0.8, NEGRO),
        ("VALIGN", (0, 0), (-1, -1), "TOP"),
    ...
    refs_tbl = Table(
        rows,
        colWidths=[16 * mm, 22 * mm, TW - 16 * mm - 22 * mm - 42 * mm - 25 * mm, 42 * mm, 25 * mm]
    )
